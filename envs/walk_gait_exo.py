...
class WalkEnvV4Multi(BaseV0):
    ...
    def _setup(self,
        ...
        # 1) 初始化外骨骼髋扭矩 CPG
        self.exo_cpg = HipAngleCPG(PaperParams(), dt=self.dt)
        ...
        # 5) 动作空间
        # - joint:      [muscle(n_m), exo(5)]
        # - exo-only:   [exo(5)]        (muscle 由 human_policy 产生)
        # - freeze_exo = True：RL 输出 muscle(n_m)，exo(5) 由 exo_policy 产生
        # - human-only: [muscle(n_m)]   (exo 由 exo_policy 产生)
        if self.freeze_human:
            act_dim = self.n_exo_act
        elif self.freeze_exo:
            act_dim = self.n_muscle_act
        else:
...
# === 一些工具函数（直接沿用 V3） ===
    ...
    def _get_imit_pose_rew(self, sigma=20.0):
        ...
        return rew
    # ----------------- 速度模仿奖励 -----------------
    def _get_imit_vel_rew(self, sigma=10.0):
