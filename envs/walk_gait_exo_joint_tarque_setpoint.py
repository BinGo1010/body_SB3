...
class WalkEnvV4Multi(BaseV0):
    ...
    - 动作：连续 Box 向量
        a = [ a_human (n_muscle_act), a_exo (2维髋关节扭矩: L,R) ]
    """
    # 人体观测
    ...
    def _setup(self,
        ...
        # 4) exo 动作维度：2维左右髋关节扭矩（PPO 输出，[-1,1] 归一化后映射到 actuator ctrlrange 或对称扭矩上限）
        self.n_exo_act = 2
        # 扭矩映射模式：
        # - 'ctrlrange'：[-1,1] 线性映射到 XML actuator ctrlrange
        # - 'symmetric'：tau = a * exo_tau_max（对称限幅，exo_tau_max 默认取 ctrlrange 的最大绝对值）
        self.exo_torque_map = str(kwargs.pop('exo_torque_map', 'ctrlrange'))
        self.exo_tau_max = float(kwargs.pop('exo_tau_max', 0.0))  # symmetric 模式下可显式指定 (Nm)，0表示自动从ctrlrange推断
...
# ==================== 外骨骼奖励相关 ======================
    def _get_exo_track_reward(self) -> float:
        """EXO 跟踪奖励（direct-torque 基线）。

        该基线中 EXO 动作为“直接扭矩”，不再存在显式的角度/速度参考轨迹（setpoint）。
        为避免引入与控制结构不匹配的额外监督信号，本项默认返回常数 1.0。

        建议在 YAML 中将 exo_track 权重设为 0，使其不影响训练。
        """
        return 1.0
...
# ========== Human policy helpers ==========
    ...
    def _predict_exo_cpg_cmd(self) -> 'np.ndarray':
        ...
        # 说明：本环境 EXO 动作为 2 维扭矩命令（归一化[-1,1]）；不再做旧版 5/6 维 CPG 参数兼容转换。
    ...
    def step(self, a, **kwargs):
        ...
        动作向量约定（EXO 为“直接髋扭矩”，PPO 直接输出左右髋扭矩命令）：
    
        - freeze_human = False 且 freeze_exo = False（联合训练）：
            a = [muscle_cmd(n_muscle_act), tau_raw_L, tau_raw_R]
    
        - freeze_human = True（冻结人体，只训练外骨骼）：
            a = [tau_raw_L, tau_raw_R]
            肌肉动作 muscle_cmd 由预训练人体策略 human_policy 产生。
    
        - freeze_exo = True（冻结外骨骼，只训练人体）：
            a = [muscle_cmd(n_muscle_act)]
            外骨骼动作 exo_act 由预训练/固定 exo 策略在环境内部产生（其输出同样为 2 维扭矩命令）。
    
        其中 tau_raw_* 的取值范围建议为 [-1, 1]（由 PPO 输出），会线性映射到 XML actuator ctrlrange（或对称扭矩上限）:
            exo_hip_l / exo_hip_r ctrlrange（Nm）
        """
        self._ensure_exo_pd_cache()
        ...
        self._debug_cache = {
            ...
            "debug/tau_raw_L":   float("nan"),
            "debug/tau_raw_R":   float("nan"),
            "debug/q_L":         float("nan"),
            "debug/q_R":         float("nan"),
        ...
        # ========== 2) EXO 动作：[-1,1] -> 髋扭矩（Nm） ==========
        # 映射优先级：
        #  - exo_torque_map='ctrlrange' : 线性映射到 XML actuator ctrlrange
        #  - exo_torque_map='symmetric': tau = a * exo_tau_max（对称限幅；exo_tau_max=0 则自动从 ctrlrange 推断）
        ...
        tau_raw_L = float(exo_act[0])
        tau_raw_R = float(exo_act[1])
        self._debug_cache["debug/tau_raw_L"] = tau_raw_L
        self._debug_cache["debug/tau_raw_R"] = tau_raw_R

        mode = str(getattr(self, "exo_torque_map", "ctrlrange")).lower()

        if mode == "symmetric":
            tau_max = float(getattr(self, "exo_tau_max", 0.0))
            if tau_max <= 0.0:
                tau_max = max(abs(loL), abs(hiL), abs(loR), abs(hiR))
                tau_max = max(tau_max, 1e-6)
            tau_L = float(np.clip(tau_raw_L * tau_max, loL, hiL))
            tau_R = float(np.clip(tau_raw_R * tau_max, loR, hiR))
        else:
            # ctrlrange 线性映射（允许 lo/hi 非对称）
            tau_L = loL + (tau_raw_L + 1.0) * 0.5 * (hiL - loL)
            tau_R = loR + (tau_raw_R + 1.0) * 0.5 * (hiR - loR)
            tau_L = float(np.clip(tau_L, loL, hiL))
            tau_R = float(np.clip(tau_R, loR, hiR))

        # ========== 3) 读当前 exo 髋关节状态（用于 debug / 可选功率门控） ==========
        ...
        # ========== 4) （可选）扭矩一阶低通，抑制 PPO 高频抖动 ==========
        alpha_tau = float(getattr(self, "exo_tau_alpha", 0.0))  # 0表示关闭
        if alpha_tau > 0.0:
            if not hasattr(self, "_exo_tau_filt"):
                self._exo_tau_filt = np.array([tau_L, tau_R], dtype=np.float32)
            else:
                cur = np.array([tau_L, tau_R], dtype=np.float32)
                self._exo_tau_filt = (1.0 - alpha_tau) * self._exo_tau_filt + alpha_tau * cur
            tau_L, tau_R = float(self._exo_tau_filt[0]), float(self._exo_tau_filt[1])
